...
from core.domains.models import DOMAIN_UNKNOWN, TableDomainAssignment

# Minimum number of Unknown tables a pattern must cover to be suggested.
_MIN_SUPPORT: int = 3


@dataclass
class LearnedDomainRule:
    id: int
    source_id: int
...
def _matches(rule: LearnedDomainRule, table_name: str, schema_name: str) -> bool:
...
def apply_learned_rules(
    ...
    confidence).  Returns the first matching TableDomainAssignment, or None
        ...
        if _matches(rule, table_name, schema_name):
            return TableDomainAssignment(
                ...
                evidence=[
                    ...
                    f"'{rule.pattern_value}' → {rule.domain}"
                ],
                competing_domains=[],
            )
    return None
...
def suggest_domain_rules(
    ...
    """Analyse Unknown table assignments and produce rule suggestions.

    Args:
        unknown_tables: Dicts with at least table_name, schema_name, table_fqn,
            and optionally competing_domains (list of dicts with 'domain' key).
    ...
    Returns:
        ...
        (SCHEMA > PREFIX > SUFFIX > TOKEN).  Each entry:
            ...
            example_tables, suggested_domain, suggested_confidence.
    """
    # Accumulate table names per pattern value
    prefix_tables: dict[str, list[str]] = {}
    suffix_tables: dict[str, list[str]] = {}
    token_tables:  dict[str, list[str]] = {}
    schema_tables: dict[str, list[str]] = {}

    # Competing domain tallies per pattern value (for domain inference)
    prefix_domains: dict[str, Counter] = {}
    suffix_domains: dict[str, Counter] = {}
    token_domains:  dict[str, Counter] = {}
    schema_domains: dict[str, Counter] = {}
    ...
    for t in unknown_tables:
        ...
        competing = t.get("competing_domains") or []
        toks      = _tokenize(name)

        if toks:
            p = toks[0]
            prefix_tables.setdefault(p, []).append(name)
            _tally(prefix_domains, p, competing)

        if len(toks) >= 2:
            s = toks[-1]
            suffix_tables.setdefault(s, []).append(name)
            _tally(suffix_domains, s, competing)

        for tok in toks:
            token_tables.setdefault(tok, []).append(name)
            _tally(token_domains, tok, competing)

        if schema:
            schema_tables.setdefault(schema, []).append(name)
            _tally(schema_domains, schema, competing)

    def _infer_domain(domain_ctr: dict[str, Counter], key: str) -> tuple[str, float]:
        ctr = domain_ctr.get(key, Counter())
        if not ctr:
            return DOMAIN_UNKNOWN, 0.75
        top_domain, top_count = ctr.most_common(1)[0]
    ...
    # Prefixes that already meet the threshold — used to suppress redundant
    # TOKEN suggestions for the same value (PREFIX is always more specific).
    strong_prefix_vals = {
    ...
    for bucket, domain_bucket, ptype in (
        (schema_tables, schema_domains, "SCHEMA"),
        (prefix_tables, prefix_domains, "PREFIX"),
        (suffix_tables, suffix_domains, "SUFFIX"),
        (token_tables,  token_domains,  "TOKEN"),
    ):
        for val, tables in bucket.items():
            ...
            domain, confidence = _infer_domain(domain_bucket, val)
            suggestions.append({
                "pattern_type":         ptype,
                "pattern_value":        val,
